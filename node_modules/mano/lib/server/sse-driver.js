...
module.exports = function (req, res) {
	...
	Client.get(req).done(function (resolvedClient) {
		...
		debugClient("%s SEE connected", client._id_);
		client.onConnect();
	...
	req.on('close', onClose = function () {
		...
		if (client.connection === connection) {
			...
			debugClient("%s SEE disconnected", client._id_);
...
Object.defineProperties(Connection.prototype, assign({
	send: d(function (action, data) {
		var id = uuid(), def;
		debug("%s server -> client %s %s", this.client._id_, action, id);
		if (isArray(data)) {
			if (data.length > 20) {
		...
		setTimeout(function () {
			if (!def.resolved) {
				debugClient("%s SSE Reception not confirmed after 2 minutes %s", this.client._id_, id);
