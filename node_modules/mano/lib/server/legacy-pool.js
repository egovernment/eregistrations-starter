...
var resolveCookies = function (req, res) {
	...
	some(cookies, function (value, name) {
		if (startsWith.call(name, 'authenticated')) {
			authenticated = value;
			return true;
		}
	});
	id = 'legacy-' + (req.$appName || 'main');
	if (req.$addonLegacyId) id += '-' + req.$addonLegacyId;
	if (authenticated) id += '-' + authenticated;
	cookies.id = id;
...
module.exports = function (env) {
	...
	send = function (req, res, message/*, options*/) {
		...
		debug("request html %s %o %o", id, message.cookies, postMessage);
	...
	middleware = function (req, res/*, options*/) {
		var cookies = resolveCookies(req, res), options = Object(arguments[2]), message;
