...
var toArray          = require('es5-ext/array/to-array')
  ...
  , userAppMap       = require('./user-app-access-map')
...
Client = module.exports = function (ua, id, appId, clientDataMap) {
	...
	if (startsWith.call(id, 'legacy_')) {
		this.isLegacy = true;
		appId = id.split('_', 3)[2];
	}
...
Client.get = function (req) {
	...
	if (inProgress[id]) {
		return inProgress[id](function (client) { return client.adaptRequest(req)(client); });
	}
	if (Client.hasOwnProperty(id)) {
		return Client[id].initPromise(function (client) { return client.adaptRequest(req)(client); });
	}
	...
	})(function (client) { return client.adaptRequest(req)(client); });
...
ee(Object.defineProperties(Client.prototype, assign({
	...
	init: d(function (appId) {
		debug("%s initialisation %s [%s]", this._id_, this.ua, ++counter);
		this.appId = appId;
	...
	adaptRequest: d(function (req) {
		if (!req.$user) return deferred(this.logout());
		return deferred(this.login(req.$user));
	}),
	...
	login: d(function (userId) {
		...
		this._userId = userId;
		debug("%s login %s", this._id_, userId);
		userAppMap.on(userId, this.setAppId);
		return userAppMap.get(userId)(this.setAppId);
	}),
	logout: d(function () {
		if (this.isLegacy) return;
		if (!this._userId) return;
		debug("%s logout", this._id_, this._userId);
	...
	appId: d.gs(function () { return this._appId; }, function (appId) {
		...
		if (old === nu) return;
		if (nu && (!nu.promise || !nu.promise.resolved)) nu.flush();
		debug("%s update appId to %s, connected: %s", this._id_, this._appId, Boolean(this.connection));
	...
	setAppId: d(function (appId) {
		this.appId = (unserializeValue(appId) || this._userId);
		return this.appFragmentPromise;
	}),
