...
module.exports = function (req, res, next) {
	...
	promise.done(function (content) {
		// Send script
		res.end(content);
		debug("%s %s Webmake OK [%s modules from %s packages in %ss]", req.$clientId, req.url,
			promise.parser.modulesFiles.length, count(promise.parser.packages),
			(promise.time / 1000).toFixed(2));
	}, function (err) {
		debug("%s %s Webmake error", req.$clientId, req.url);
