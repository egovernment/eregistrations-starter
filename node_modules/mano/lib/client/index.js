...
module.exports = function (options) {
	...
	var getCookie = require('dom-ext/html-document/#/get-cookie'), wasRefreshed;
	userId = getCookie.call(document, 'authenticated' + location.port);
		...
		if (appName !== 'public') {
			...
			if (!wasRefreshed) {
				require('dom-ext/html-document/#/set-cookie')
					.call(document, 'authRefreshCheck', '1', { age: 60 });
				location.reload();
				return;
			}
	...
	var bpApps = { registration: true, 'business-update': true, 'user-submitted': true };
	loadView = function () {
		...
		if (userId) {
			...
			if (!user || (bpApps[appName] && !user.currentBusinessProcess)) {
