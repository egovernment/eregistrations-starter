...
var serializeForError = function (name, value) {
	var str = name + ':\n';
	try {
		str += 'string representation: ' + String(value) + '\n';
	} catch (ignore) {}
	try {
		str += 'json representation: ' + JSON.stringify(value) + '\n';
	} catch (ignore) {}
	if (value) {
		if (value.outerHTML) str += 'outerHTML: ' + value.outerHTML + '\n';
		else if (value.innerHTML) str += 'innerHTML: ' + value.innerHTML + '\n';
	}
	return str;
};
...
Object.defineProperties(observable, {
	toDOM: d(function (document) {
		...
		this.on('change', function (event) {
			var parent, value = event.newValue, errorInfo;
			if (current) {
				if (isArray(current)) {
			...
			if (!parent) {
				...
				errorInfo = serializeForError('Event', event) + '\n';
				console.log("New Value", value);
				errorInfo += serializeForError('New Value', value) + '\n';
				console.log("Observable", this);
				errorInfo += serializeForError('Observable', this) + '\n';
				console.log("Current DOM", current);
				errorInfo += serializeForError('Current DOM', current) + '\n';
