...
Input.prototype = Object.create(DOMInput.prototype, assign({
	...
	_render: d(function () {
		...
		this.dom.setAttribute('class', 'btn-group radio-btn-group');
	}),
	...
	onChange: d(function () {
		...
		if (this.current) {
			this.listItems[this.current].classList
				.remove('btn-' + (this.classMap[this.current] || 'success'));
		}
		this.current = value;
		if (!value) return;
		this.listItems[value].classList
			.add('btn-' + (this.classMap[value] || 'success'));
	})
})));
