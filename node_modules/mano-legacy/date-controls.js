...
module.exports = $.dateControls = function (id) {
	...
	updateDefault = function () {
		var def = control.getAttribute('value') || null, nu, old, controlsToUpdate;
		...
		currentDefault = def;
		if (old) {
			controlsToUpdate = [years[old.getFullYear()], months[old.getMonth() + 1],
				days[old.getDate()]];
			controlsToUpdate.forEach(function (control) {
				if (control) {
					control.removeAttribute('selected');
				} else {
					console.error("Date control configuration/model mismatch: " +
						"cannot display date value: " + old);
				}
			});
		}
		if (nu) {
			controlsToUpdate = [years[nu.getFullYear()], months[nu.getMonth() + 1], days[nu.getDate()]];
			controlsToUpdate.forEach(function (control) {
				if (control) {
					control.setAttribute('selected', 'selected');
				} else {
					console.error("Date control configuration/model mismatch: " +
						"cannot display date value: " + nu);
				}
			});
		}
	};
