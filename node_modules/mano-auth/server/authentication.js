...
var ensureString     = require('es5-ext/object/validate-stringifiable-value')
  ...
  , now = Date.now
  , env = mano.env, userStorage = mano.dbDriver && mano.dbDriver.getStorage('user')
...
exports.verify = verify = function (email, password) {
	...
	return userEmailMap(function (map) {
		user = map.get(serializeValue(email));
		if (!user) return null;
		return userStorage.get(user + '/password')(function (data) {
			var userPassword;
			if (!data) return null;
			userPassword = unserializeValue(data.value);
			if (!userPassword) return null;
			return hash.compare(password, userPassword)(function (isOK) { return isOK ? user : null; });
		});
	});
};
